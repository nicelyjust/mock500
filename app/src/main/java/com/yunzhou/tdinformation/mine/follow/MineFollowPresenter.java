...
public class MineFollowPresenter extends WrapperPresenter<MineFollowView> {

    private static final String TAG = "MineFollowPresenter";
    private List<FollowEntity.FollowBean> mData;
    private int pageNo = 1;

    public MineFollowPresenter() {
        mData = new ArrayList<>();
    }
    ...
    public void loadData(Context context, final int loadType, int uid) {
        ...
        GetRequest<JsonResult<DataBean<FollowEntity>>> request;
        if (UserManager.getInstance().isMine(uid)) {
            request = OkGo.get( NetConstant.GET_MY_FOLLOW + UserManager.getInstance().getUid());
        } else {
            request = OkGo.get(NetConstant.GET_OTHER_FOLLOW + UserManager.getInstance().getUid() + "/" + uid);
        }
        ...
        request.headers(AppConst.Param.JWT, UserManager.getInstance().getJwt())
                ...
                .execute(new LoginCallback<JsonResult<DataBean<FollowEntity>>>(context) {
                    @Override
                    public void onSuccess(Response<JsonResult<DataBean<FollowEntity>>> response) {
                        FollowEntity dataMap = response.body().content.dataMap;
                        if (dataMap != null) {
                    ...
                    @Override
                    public void onError(Response<JsonResult<DataBean<FollowEntity>>> response) {
    ...
    private void loadFollowsSuccess(int loadType, List<FollowEntity.FollowBean> list) {
        if (!isViewNotNull()) {
            return;
        }
        mView.hideLoading();
